...
# Preset: run 5 Algorithms-domain tasks as 5 independent runs, 100 steps each (single GPU).
...
for task in "${TASKS[@]}"; do
  ...
  LOG_PATH="logs/rl_algorithmsdomain_${task}_100steps_lora_${DATE_TIME}.log"
  ...
  "${RLVR_PYTHON}" grpo_train_local.py \
    ...
    "trainer.experiment_name=AlgorithmsDomain_rl_${task}_100steps_lora_hf_seed\${seed}" \
    "$@" \
    2>&1 | tee "${LOG_PATH}"
done
