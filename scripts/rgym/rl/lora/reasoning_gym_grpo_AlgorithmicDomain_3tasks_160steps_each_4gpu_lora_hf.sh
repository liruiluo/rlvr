...
# Usage:
#   bash scripts/rgym/rl/lora/reasoning_gym_grpo_AlgorithmicDomain_3tasks_160steps_each_4gpu_lora_hf.sh
set -euo pipefail
...
for task in "${TASKS[@]}"; do
  ...
  LOG_PATH="logs/rl_4gpu_algorithmicdomain_${task}_160steps_lora_${DATE_TIME}.log"
  ...
  "${RLVR_PYTHON}" grpo_train_local.py \
    ...
    "trainer.experiment_name=AlgorithmicDomain_rl_4gpu_${task}_160steps_lora_hf_seed\${seed}" \
    "${EXTRA_OVERRIDES[@]}" \
    "$@" \
    2>&1 | tee "${LOG_PATH}"
done
