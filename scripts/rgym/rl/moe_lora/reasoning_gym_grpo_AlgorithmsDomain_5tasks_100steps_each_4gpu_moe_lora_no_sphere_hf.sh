...
# Preset: run 5 Algorithms-domain tasks as 5 independent runs, 100 steps each.
# Variant: MoE-LoRA enabled, SPHERE disabled.
# Usage:
#   bash scripts/rgym/rl/moe_lora/reasoning_gym_grpo_AlgorithmsDomain_5tasks_100steps_each_4gpu_moe_lora_no_sphere_hf.sh
set -euo pipefail
...
for task in "${TASKS[@]}"; do
  ...
  LOG_PATH="logs/rl_4gpu_algorithmsdomain_${task}_100steps_no_sphere_${DATE_TIME}.log"
  ...
  "${RLVR_PYTHON}" grpo_train_local.py \
    ...
    "trainer.experiment_name=AlgorithmsDomain_rl_4gpu_${task}_100steps_moe_lora_no_sphere_hf_seed\${seed}" \
    "$@" \
    2>&1 | tee "${LOG_PATH}"
done
