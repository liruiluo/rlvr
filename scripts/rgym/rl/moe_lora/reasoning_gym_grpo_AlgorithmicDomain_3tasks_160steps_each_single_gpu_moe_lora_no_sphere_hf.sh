...
for task in "${TASKS[@]}"; do
  ...
  LOG_PATH="logs/rl_algorithmicdomain_${task}_160steps_moe_lora_no_sphere_${DATE_TIME}.log"
  ...
  "${RLVR_PYTHON}" grpo_train_local.py \
    ...
    "trainer.experiment_name=AlgorithmicDomain_rl_${task}_160steps_moe_lora_no_sphere_hf_seed\${seed}" \
    "${EXTRA_OVERRIDES[@]}" \
    "$@" \
    2>&1 | tee "${LOG_PATH}"
done
