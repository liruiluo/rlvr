#!/usr/bin/env bash
# Single-task RL (GRPO) on Reasoning Gym with verl (single GPU).
# Preset: run 5 Algorithms-domain tasks as 5 independent runs, 100 steps each.
# Variant: MoE-LoRA enabled, SPHERE disabled.
# Usage:
#   bash scripts/rgym/rl/moe_lora/reasoning_gym_grpo_AlgorithmsDomain_5tasks_100steps_each_single_gpu_moe_lora_no_sphere_hf.sh
set -euo pipefail

source scripts/common/setup_env.sh

cd "${RLVR_REPO_ROOT}/experiments/verl_rgym"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

VISIBLE_GPUS="$("${RLVR_PYTHON}" - <<'PY'
import torch
print(torch.cuda.device_count())
PY
)"
if [[ "${VISIBLE_GPUS}" -lt 1 ]]; then
  echo "Expected >=1 visible CUDA GPU but got ${VISIBLE_GPUS}. Check CUDA_VISIBLE_DEVICES / driver setup." >&2
  exit 1
fi

TASKS=(basic_arithmetic chain_sum gcd letter_counting spell_backward)

for task in "${TASKS[@]}"; do
  ray stop -f

  mkdir -p logs
  DATE_TIME="$(date +%Y%m%d_%H%M%S)"
  LOG_PATH="logs/rl_algorithmsdomain_${task}_100steps_no_sphere_${DATE_TIME}.log"
  echo "Task=${task} Log: ${RLVR_REPO_ROOT}/experiments/verl_rgym/${LOG_PATH}"

  "${RLVR_PYTHON}" grpo_train_local.py \
    --config-path configs \
    --config-name algo/rgym/grpo_moe_lora_sphere_hf \
    -m seed=0,1,2 \
    "task=${task}" \
    reasoning_gym.dataset_size=256 \
    trainer.total_training_steps=100 \
    trainer.test_freq=20 \
    actor_rollout_ref.rollout.custom.verl_ext.use_moe_lora=true \
    actor_rollout_ref.rollout.custom.verl_ext.sphere_feature_ratio=0.0 \
    actor_rollout_ref.rollout.custom.verl_ext.sphere_gating_ratio=0.0 \
    "trainer.experiment_name=AlgorithmsDomain_rl_${task}_100steps_moe_lora_no_sphere_hf_seed\${seed}" \
    "$@" \
    2>&1 | tee "${LOG_PATH}"
done
