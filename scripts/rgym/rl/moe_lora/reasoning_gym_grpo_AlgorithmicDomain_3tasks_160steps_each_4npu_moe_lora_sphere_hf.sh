...
# Usage:
#   bash scripts/rgym/rl/moe_lora/reasoning_gym_grpo_AlgorithmicDomain_3tasks_160steps_each_4npu_moe_lora_sphere_hf.sh
set -euo pipefail
...
for task in "${TASKS[@]}"; do
  ...
  LOG_PATH="logs/rl_4npu_algorithmicdomain_${task}_160steps_moe_lora_sphere_${DATE_TIME}.log"
  ...
  "${RLVR_PYTHON}" grpo_train_local.py \
    ...
    "trainer.experiment_name=AlgorithmicDomain_rl_4npu_${task}_160steps_moe_lora_sphere_hf_seed\${seed}" \
    "${EXTRA_OVERRIDES[@]}" \
    "$@" \
    2>&1 | tee "${LOG_PATH}"
done
