...
# Usage:
#   bash scripts/rgym/crl/lora/reasoning_gym_continual_single_npu_lora_hf.sh [Hydra overrides...]
set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"

exec bash "${SCRIPT_DIR}/../moe_lora/reasoning_gym_continual_single_npu_moe_lora_sphere_hf.sh" \
  'trainer.experiment_name=continual_single_npu_lora_hf_seed${seed}' \
