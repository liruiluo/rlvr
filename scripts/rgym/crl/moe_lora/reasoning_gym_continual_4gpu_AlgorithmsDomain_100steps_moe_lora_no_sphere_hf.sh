...
# Usage:
#   bash scripts/rgym/crl/moe_lora/reasoning_gym_continual_4gpu_AlgorithmsDomain_100steps_moe_lora_no_sphere_hf.sh
set -euo pipefail
...
LOG_PATH="logs/continual_4gpu_algorithmsdomain_100steps_no_sphere_${DATE_TIME}.log"
...
"${RLVR_PYTHON}" grpo_train_local.py \
  ...
  'trainer.experiment_name=continual_4gpu_AlgorithmsDomain_100steps_moe_lora_no_sphere_hf_seed${seed}' \
  "$@" \
  2>&1 | tee "${LOG_PATH}"
