...
# Usage:
#   bash scripts/rgym/crl/moe_lora/reasoning_gym_continual_4gpu_AlgorithmicDomain_160steps_moe_lora_sphere_hf.sh
set -euo pipefail
...
"${RLVR_PYTHON}" grpo_train_local.py \
  ...
  'trainer.experiment_name=continual_4gpu_AlgorithmicDomain_160steps_moe_lora_sphere_hf_seed${seed}' \
  "$@" \
  2>&1 | tee "${LOG_PATH}"
