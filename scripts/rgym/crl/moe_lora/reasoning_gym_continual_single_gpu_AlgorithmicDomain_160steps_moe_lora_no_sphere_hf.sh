...
LOG_PATH="logs/continual_algorithmicdomain_160steps_${DATE_TIME}.log"
...
"${RLVR_PYTHON}" grpo_train_local.py \
  ...
  'trainer.experiment_name=continual_1gpu_AlgorithmicDomain_160steps_moe_lora_no_sphere_hf_seed${seed}' \
  "$@" \
  2>&1 | tee "${LOG_PATH}"
